...
export function useSwapLogic(
  ...
  // Ref to prevent re-entrant initialization
  const initializingRef = useRef(false);

  // Initialize tokens with details
  const initializeTokens = useCallback(async () => {
    if (!isMounted.current || initializingRef.current || tokensInitialized) {
      return;
    }
    ...
    try {
      ...
      // Use hardcoded defaults when API returns null (e.g. no Birdeye key)
      if (!initialInputToken) {
        initialInputToken = routeParams.inputToken || DEFAULT_SOL_TOKEN;
      }
      if (!initialOutputToken) {
        initialOutputToken = routeParams.outputToken || DEFAULT_USDC_TOKEN;
      }
  ...
  }, [userPublicKey, routeParams, tokensInitialized]);
  ...
  // Fetch balance and price when tokens are initialized and wallet is connected
  const dataFetchedRef = useRef(false);
  useEffect(() => {
    if (!tokensInitialized || !connected || !userPublicKey || !inputToken) {
    ...
    const fetchSequence = async () => {
      ...
      try {
        ...
        // Token data fetch failed silently
      }
    };

    const timer = setTimeout(fetchSequence, 200);
    return () => clearTimeout(timer);
  }, [tokensInitialized, connected, userPublicKey, inputToken]);
