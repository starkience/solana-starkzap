...
export async function fetchTokenPrice(tokenInfo: TokenInfo | null): Promise<number | null> {
  ...
  try {
    // Try Jupiter API first (free, no key needed)
    try {
      const jupResponse = await fetch(`https://api.jup.ag/price/v2?ids=${tokenInfo.address}`);
      if (jupResponse.ok) {
        const jupData = await jupResponse.json();
        if (jupData?.data?.[tokenInfo.address]?.price) {
          const price = parseFloat(jupData.data[tokenInfo.address].price);
          return price;
        }
      }
    } catch (err) {
      // Jupiter fetch failed silently
    }

    // Try Birdeye only if API key is configured
    if (BIRDEYE_API_KEY) {
      try {
...
const BIRDEYE_HEADERS = BIRDEYE_API_KEY ? {
  'accept': 'application/json',
  'X-API-KEY': BIRDEYE_API_KEY,
} : null;
...
export async function fetchTokenMetadata(tokenAddress: string): Promise<TokenInfo | null> {
  if (!BIRDEYE_HEADERS) return null;
  try {
